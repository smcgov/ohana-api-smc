...
describe "GET 'nearby'" do
  ...
  it 'is paginated' do
    get api_location_nearby_url(
      @loc, page: 2, per_page: 1, radius: 5, subdomain: api_subdomain
    )
    expect(json.first['name']).to eq('Belmont Farmers Market')
  end

  it 'returns a summarized JSON representation' do
    get api_location_nearby_url(@loc, radius: 2, subdomain: api_subdomain)
  ...
  it 'only returns active locations' do
    ...
    get api_location_nearby_url(@loc, per_page: 2, radius: 5, subdomain: api_subdomain)
    expect(json.length).to eq(1)
  end

  context 'with no radius' do
    it 'displays nearby locations within 0.5 miles' do
      get api_location_nearby_url(@loc, subdomain: api_subdomain)
      expect(json).to eq([])
    end
  end
  ...
  context 'with invalid radius' do
    it 'returns an invalid radius message' do
      get api_location_nearby_url(@loc, radius: 'script', subdomain: api_subdomain)
  ...
  context 'when the location has no coordinates' do
    it 'returns empty array' do
      ...
      get api_location_nearby_url(no_address, subdomain: api_subdomain)
      expect(json).to eq([])
    end
  end
end
