...
describe 'GET /locations/:location_id/services' do
  context 'when location has services' do
    before :each do
      create_service
      get api_location_services_url(@location, subdomain: api_subdomain)
    end
    ...
    it 'serializes all service attributes and associations' do
      expect(json.first.keys).to eq(
        %w[id accepted_payments alternate_name application_process audience
  ...
  context "when location doesn't have services" do
    before :each do
      loc = create(:location)
      get api_location_services_url(loc, subdomain: api_subdomain)
    end
