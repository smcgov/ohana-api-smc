...
describe 'Pagination Headers' do
  before(:each) do
    @prefix = api_search_index_url(subdomain: api_subdomain)
  end
  ...
  context 'when on page 1 of 2' do
    ...
    before(:each) do
      get api_search_index_url(
        keyword: 'jobs', per_page: 1, subdomain: api_subdomain
      )
    end
  ...
  context 'when on page 2 of 2' do
    ...
    before(:each) do
      get api_search_index_url(
        keyword: 'jobs', page: 2, per_page: 1, subdomain: api_subdomain
      )
    end
  ...
  context 'when on page higher than max' do
    ...
    before(:each) do
      get api_search_index_url(
        keyword: 'vrs', page: 3, subdomain: api_subdomain
      )
    end
  ...
  context 'when there is only one page of search results' do
    it 'does not return a Link header' do
      create(:location)
      get api_search_index_url(
        keyword: 'jobs', subdomain: api_subdomain
      )
  ...
  context 'when there are no search results' do
    it 'returns one rel=last link with page=0' do
      create(:location)
      get api_search_index_url(keyword: 'foo', subdomain: api_subdomain)
      expect(headers['Link']).
